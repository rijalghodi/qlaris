...
export function ProductList() {
  ...
  if (filteredProducts.length === 0) {
    return (
      // REPLCAE WITH empty
      <div className="flex flex-col items-center justify-center py-12 text-center animate-in fade-in slide-in-from-bottom-4 ease-in duration-300">
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4 justify-start">
        {filteredProducts.map((product) => (
          <ProductCard
            key={product.id}
            product={product}
            className="animate-in fade-in slide-in-from-bottom-4 ease-in duration-300"
          />
        ))}
      </div>
    </>
