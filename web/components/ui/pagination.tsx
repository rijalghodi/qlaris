...
function Pagination({ page, totalPage, onPageChange }: PaginationProps) {
  ...
  const getPageNumbers = () => {
    ...
    if (totalPage <= 7) {
      ...
      if (page <= 3) {
        // Near the start
        pages.push(2, 3, 4, "ellipsis", totalPage);
      } else if (page >= totalPage - 2) {
        // Near the end
        pages.push("ellipsis", totalPage - 3, totalPage - 2, totalPage - 1, totalPage);
      } else {
        // In the middle
        pages.push("ellipsis", page - 1, page, page + 1, "ellipsis", totalPage);
      }
    }
